...
function Habits({habits,onAdd,onUpdate,onDelete,onToggleDay,onChangeDays}){
  ...
  {habits.length===0?<div className="panel habitEmpty"><div className="habitEmptyIcon"><CheckCircle2 size={22}/></div><h2>Start your first habit</h2><p>Add up to 6 habits and choose anywhere from 5 to 100 days.</p><button type="button" className="primary" onClick={onAdd}><Plus size={16}/> Add first habit</button></div>:<div className="habitList">{habits.map((h,index)=>{const done=(h.completed||[]).length;const progress=Math.round((done/h.days)*100);return <div className="panel habitCard" key={h.id} style={{"--habit-index":index}}>
    <div className="habitCardHead">
      <div className="habitTitleWrap">
       ...
       {editingId===h.id?<input autoFocus className="habitNameInput" value={draft} maxLength={40} onChange={e=>setDraft(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')commitEdit(h.id);if(e.key==='Escape'){setEditingId(null);setDraft('')}}} onBlur={()=>commitEdit(h.id)}/>:<h2>{h.name}</h2>}
      </div>
      <div className="habitCardActions">
       <label className="habitDaysControl"><span>Days</span><input type="number" min="5" max="100" value={h.days} onChange={e=>onChangeDays(h.id,e.target.value)} aria-label={`Number of days for ${h.name}`}/></label>
    ...
    <div className="habitMeta"><span>{done} of {h.days} days completed</span><b>{progress}%</b></div>
    <div className="habitProgress"><span style={{width:`${progress}%`}}/></div>
