...
import {LayoutDashboard,CheckSquare,BookOpen,CalendarDays,CalendarClock,Timer,Plus,Search,ChevronDown,Trash2,Check,Clock3,Target,Flame,LogOut,Menu,X,ArrowUpRight,Play,Pause,RotateCcw,Settings,Pencil,Sun,Moon,Monitor,CircleUser,Palette,Bell,SlidersHorizontal,ShieldCheck,FileText,Upload,Download,ExternalLink,File,FilePlus2,ChevronRight} from 'lucide-react';
import './styles.css';
...
function App(){
 ...
 async function deleteSubject(subject){
  ...
  try{await deleteSubjectLocalData(subject.id)}catch{}
 ...
 async function logSession(minutes,subjectId,date,startTime,endTime){const mins=Math.max(1,Math.round(Number(minutes)||1));const row={user_id:user?.id,subject_id:subjectId||null,minutes:mins,session_date:date||localDateKey(new Date()),start_time:startTime||null,end_time:endTime||null,note:''};if(cloudReady&&user){const {data,error}=await supabase.from('study_sessions').insert(row).select().single();if(error)throw error;if(data)setSessions(x=>[data,...x])}else setSessions(x=>[{...row,id:uid()},...x]);setRunning(false);setSessionModal(false);setSessionEdit(null)}
 async function saveSession(session,minutes,subjectId,date,startTime,endTime){if(!session?.id){return logSession(minutes,subjectId,date,startTime,endTime)}const mins=Math.max(1,Math.round(Number(minutes)||1));const patch={subject_id:subjectId||null,minutes:mins,session_date:date||localDateKey(new Date()),start_time:startTime||null,end_time:endTime||null};if(cloudReady&&user){const {data,error}=await supabase.from('study_sessions').update(patch).eq('id',session.id).select().single();if(error){alert(`Could not update this logged session. ${error.message}`);return}setSessions(x=>x.map(s=>s.id===session.id?(data||{...s,...patch}):s))}else setSessions(x=>x.map(s=>s.id===session.id?{...s,...patch}:s));setSessionEdit(null);setSessionModal(false)}
...
function SessionModal({subjects,defaultMinutes,onClose,onSave,initialSession=null}){
 const editing=Boolean(initialSession);
 const now=new Date();
 const initialDate=initialSession?.session_date||localDateKey(now);
 const [subjectId,setSubjectId]=useState(initialSession?.subject_id||'');
 const [date,setDate]=useState(initialDate);
 const [startTime,setStartTime]=useState(initialSession?.start_time||`${String(now.getHours()).padStart(2,'0')}:${String(now.getMinutes()).padStart(2,'0')}`);
 const [endTime,setEndTime]=useState(initialSession?.end_time||'');
 const [minutes,setMinutes]=useState(String(initialSession?.minutes||defaultMinutes||25));
 ...
 return <div className="overlay sessionOverlay" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}>
  <form className="modal sessionModal" onSubmit={submit} onMouseDown={e=>e.stopPropagation()}>
   <div className="modalHead"><div><span className="eyebrow">{editing?'EDIT LOGGED TIME':'FOCUS LOG'}</span><h2>{editing?'Edit logged session':'Log a session'}</h2></div><button className="iconbtn" type="button" onClick={onClose} aria-label="Close session dialog"><X/></button></div>
   <p className="settingsHint">{editing?'Update the date, subject, start time, or end time. ORBIT recalculates the duration automatically.':'Record exactly when you studied. ORBIT calculates the duration from your start and end time.'}</p>
   <div className="sessionFields">
    <label><span>Date</span><DateField value={date} onChange={setDate} label="Session date"/></label>
    ...
    <label><span>Start time</span><TimeField value={startTime} onChange={setStartTime} label="Start time"/></label>
    <label><span>End time</span><TimeField value={endTime} onChange={setEndTime} label="End time"/></label>
   ...
   {calculated&&<div className="sessionCalc"><Clock3 size={16}/><span><b>{formatLoggedTime(calculated)}</b> · {formatTimeValue(startTime)} – {formatTimeValue(endTime)}{endTime<=startTime?' · crosses midnight':''}</span></div>}
   <div className="modalActions"><button type="button" className="secondary" onClick={onClose}>Cancel</button><button type="submit" className="primary"><Check size={16}/> {editing?'Save changes':'Log session'}</button></div>
  </form>
 </div>
}
...
function SessionHistoryModal({date,sessions,subjects,onDateChange,onDelete,onEdit,onClose}){
