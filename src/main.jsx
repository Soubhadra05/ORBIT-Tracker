...
function App(){
 ...
 function addHabit(){
  ...
  const now=Date.now();
  setHabits(prev=>prev.length>=5?prev:[...prev,{id:uid(),name:'New habit',days:30,completed:[],createdAt:now}]);
 }
 function updateHabit(id,patch){setHabits(prev=>prev.map(h=>h.id===id?{...h,...patch}:h))}
 function deleteHabit(id){
  const habit=habits.find(h=>h.id===id);
  if(habit&&!window.confirm(`Delete \"${habit.name}\"? This will remove its progress.`))return;
  setHabits(prev=>prev.filter(h=>h.id!==id));
 }
 function toggleHabitDay(id,day){setHabits(prev=>prev.map(h=>{if(h.id!==id)return h;const completed=new Set(h.completed||[]);if(completed.has(day))completed.delete(day);else completed.add(day);return {...h,completed:[...completed].filter(n=>n>=1&&n<=h.days).sort((a,b)=>a-b)}}))}
...
function getHabitStreaks(completed,days){
 const set=new Set((completed||[]).filter(day=>day>=1&&day<=days));
 let best=0,run=0;
 for(let day=1;day<=days;day++){
  if(set.has(day)){run+=1;best=Math.max(best,run)}
  else run=0;
 }
 let current=0;
 if(set.size){
  let last=Math.max(...set);
  while(set.has(last)){current+=1;last-=1}
 }
 return {current,best};
}
function Habits({habits,onAdd,onUpdate,onDelete,onToggleDay,onChangeDays,onReset,onCompleteNext}){
 ...
 const beginDaysEdit=h=>setDaysDrafts(prev=>({...prev,[h.id]:String(h.days)}));
 const setDaysDraft=(id,value)=>setDaysDrafts(prev=>({...prev,[id]:value}));
 const commitDays=(h)=>{
  if(skipDaysBlur.current.has(h.id)){skipDaysBlur.current.delete(h.id);return}
  const raw=daysDrafts[h.id] ?? String(h.days);
 ...
 return <section className="page habitsPage">
  <div className="sectionTop">
   ...
   <button type="button" className="primary habitAddTop" onClick={onAdd} disabled={habits.length>=5}><Plus size={16}/> Add habit <span>{habits.length}/5</span></button>
  </div>
  {habits.length===0?<div className="panel habitEmpty"><div className="habitEmptyIcon"><CheckCircle2 size={22}/></div><h2>Start your first habit</h2><p>Add up to 5 habits and choose anywhere from 5 to 100 days.</p><button type="button" className="primary" onClick={onAdd}><Plus size={16}/> Add first habit</button></div>:
   <>
    <div className="habitSwitcher" aria-label="Choose a habit">
     <div className="habitSwitcherHead"><div><span>YOUR HABITS</span><small>Jump between habits without scrolling through every plan.</small></div><b>{habits.length}/5 active</b></div>
     <div className="habitSwitcherGrid" style={{"--habit-count":Math.min(5,Math.max(1,habits.length))}}>
      {habits.map((h,index)=>{
